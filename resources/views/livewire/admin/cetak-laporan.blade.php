...
<body>
    ...
    <div class="statistics">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value currency">Rp {{ number_format($data->where('denda_dibayar', true)->sum('denda'), 0, ',', '.') }}</div>
        </div>
        @endif
    </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        @if($item->denda > 0)
                            <div>
                                <span class="text-danger currency">Rp {{ number_format($item->denda, 0, ',', '.') }}</span>
                    ...
                    <td>
                        @php
                            $lamaPinjam = \Carbon\Carbon::parse($item->tanggal_pinjam)->diffInDays(\Carbon\Carbon::parse($item->tanggal_kembali));
                        @endphp
                        {{ $lamaPinjam }} hari
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        @endphp
                        <div>
                            <span class="text-danger"><strong>{{ $fineInfo['seconds_overdue'] }} detik</strong></span><br>
                            <small>Batas: {{ $fineInfo['due_date'] }}</small>
                        </div>
                    </td>
                    <td>
                        <div>
                            <span class="text-danger currency">Rp {{ number_format($item->denda, 0, ',', '.') }}</span><br>
                            <small>@ Rp {{ number_format($fineInfo['fine_per_second'], 0, ',', '.') }}/detik</small>
                        </div>
                    </td>
                    <td>
                        @if($item->denda_dibayar)
